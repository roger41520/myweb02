                ...
                <table class="w-full table-fixed">
                    <thead>
                        <tr class="bg-gray-100">
                            ...
                            <th class="px-4 py-2 border">CRUD功能</th>
                    ...
                    <tbody>
                        @if(!empty($articles))
                            @foreach($articles as $row)
                                <tr>
                                    <td class="px-4 py-2 border">{{ $row->產品名稱 }}</td>
                                    <td class="px-4 py-2 border">{{ $row->產品資訊 }}</td>
									<td class="px-4 py-2 border">{{ $row->產品售價 }}</td>
                                    <td class="px-4 py-2 border">{{ $row->目前庫存 }}</td>
                                    <td class="px-4 py-2 border">
                                        <form action="{{ route('articles.destroy', $row->id) }}" method="POST">
                                            <a href="{{ route('articles.show', $row->id) }}" class="inline-flex items-center px-4 py-2 mx-2 text-xs font-semibold tracking-widest text-white uppercase transition duration-150 ease-in-out bg-gray-800 border border-transparent rounded-md hover:bg-gray-700 active:bg-gray-900 focus:outline-none focus:border-gray-900 focus:shadow-outline-gray disabled:opacity-25">
                                                Show
                                            </a>
                                            <a href="{{ route('articles.edit', $row->id) }}" class="inline-flex items-center px-4 py-2 mx-2 text-xs font-semibold tracking-widest text-white uppercase transition duration-150 ease-in-out bg-gray-800 border border-transparent rounded-md hover:bg-gray-700 active:bg-gray-900 focus:outline-none focus:border-gray-900 focus:shadow-outline-gray disabled:opacity-25">
                                                Edit
                                            </a>
                                            @csrf
                                            @method('DELETE')
                        ...
                        @else
                        <tr>
                            <td class="px-4 py-2 border text-red-500" colspan="3">No articles found.</td>
                        </tr>
                        @endif
                    </tbody>
                </table>
